...
# create biscale
bisc_tws_pop <- bi_class(grace,
                         x = population, y = delta_wd_cm_yr,
                         style = "quantile", dim=n_quantile)

# turn polygons into an sf object
bisc_tws_pop_sf <- create_tile_sf_attr(df_name=bisc_tws_pop,
                                          lonvar="lon",
                                          latvar="lat",
                                          widthvar="lonspan",
                                          heightvar="latspan")
...
# map
bimap_tws_pop <- ggplot()+
  geom_sf(data=bisc_tws_pop_sf,
          aes(fill=bi_class),
          color=NA,
          show.legend = F)+
...
legend_tws_pop <- bi_legend(pal = col_pal,
                               dim=n_quantile,
                               xlab = "Larger Population ",
                               ylab = "Water Gain ",
                               size = 10)

finalplot_tws_pop <- ggdraw() +
  draw_plot(bimap_tws_pop, 0, 0, 1, 1) +
  draw_plot(legend_tws_pop, -0.08, 0.15, 0.35, 0.35)

ggsave(finalplot_tws_pop,
       filename="results/fig2c.png",
       height=6,
       width=12,
       units = "in",
       dpi=300)
